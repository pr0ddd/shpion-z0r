...
export const useStreamViewStore = create<StreamViewState>((set) => ({
  ...
  fullscreen: false,
  setActiveStream: (sid) =>
    set({ activeStreamSid: sid, multiView: false, selectedSids: [] }),
  setMultiView: (on, initialSid = null) =>
    set((s) => ({
      multiView: on,
      activeStreamSid: null,
      selectedSids: on
        ? initialSid
          ? Array.from(new Set([...s.selectedSids, initialSid]))
          : s.selectedSids
        : [],
    })),
  addToMultiView: (sid) =>
    set((s) => ({
      selectedSids: s.selectedSids.includes(sid)
        ? s.selectedSids
        : [...s.selectedSids, sid],
      multiView: true,
      activeStreamSid: null,
    })),
  removeFromMultiView: (sid) =>
    set((s) => {
  ...
  toggleFullscreen: () =>
    set((s) => ({
      ...
      chatSidebar: s.fullscreen ? s.chatSidebar : false,
    })),
  resetView: () =>
    set({
      activeStreamSid: null,
