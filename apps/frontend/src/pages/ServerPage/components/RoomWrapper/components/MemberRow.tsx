...
const MemberRowInner: React.FC<MemberRowProps> = ({
  participant,
  user,
  isDeafened,
}) => {
  ...
  const popupState = usePopupState({
    variant: 'popover',
    popupId: `preview-${participant.sid}`,
  });

  const { user: me } = useAuth();
  const listeningSelf = useServerStore(
    (s) => (me?.id ? s.listeningStates[me.id] : true) ?? true
  );
  ...
  const preload = (
    <Box sx={{ display: 'none' }}>
      {participantScreenShares.map((track) => (
        <ScreenSharePreview
          key={`preload-${track.publication.trackSid}`}
          trackRef={track}
          staticImage
        />
      ))}
  ...
  const participantSelected = useMemo(
    () =>
      participantScreenShares.some((tr) =>
        selectedSids.includes(tr.publication.trackSid)
      ),
    [participantScreenShares, selectedSids]
  );
  ...
  return (
    ...
    >
      {audioTrack && (
        <AudioTrack trackRef={audioTrack} volume={effectiveVolume} />
      )}
      ...
      {participantScreenShares.length > 0 && (
        ...
        >
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              ...
              <React.Fragment key={track.publication.trackSid}>
                ...
                >
                  ...
                  >
                    <Chip
                      label={(
                        track.publication.trackName ?? `Стрим №${idx + 1}`
                      ).slice(0, 30)}
                      size="small"
                      sx={{
                        bgcolor: 'rgba(44, 44, 44, 0.67)',
                        color: '#fff',
                        fontWeight: 600,
                        pointerEvents: 'none',
                      }}
                    ...
                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                      <IconButton
                        onClick={(e) => {
                          e.stopPropagation();
                          openStream(track.publication.trackSid);
                        }}
                        sx={{
                      ...
                      {selectedSids.includes(track.publication.trackSid) ? (
                        <IconButton
                          onClick={(e) => {
                            ...
                            if (isRemotePublication(track.publication)) {
                              void track.publication.setSubscribed(false);
                            }
                            store.removeFromMultiView(
                        ...
                        <IconButton
                          onClick={(e) => {
                            ...
                            const store = useStreamViewStore.getState();
                            if (isRemotePublication(track.publication)) {
                              void track.publication.setSubscribed(true);
                            }
                            store.addToMultiView(track.publication.trackSid);
                          }}
                          sx={{
                ...
                {idx !== participantScreenShares.length - 1 && (
            ...
            <Button
              variant="contained"
              color={participantSelected ? 'error' : 'success'}
              size="small"
              fullWidth
              onClick={(e) => {
                e.stopPropagation();
                ...
                if (participantSelected) {
                  ...
                  participantScreenShares.forEach((tr) => {
                    if (isRemotePublication(tr.publication))
                      void tr.publication.setSubscribed(false);
                    store.removeFromMultiView(tr.publication.trackSid);
                  });
                } else {
                  const firstSid =
                    participantScreenShares[0]?.publication.trackSid;
                  store.setMultiView(true, firstSid);
                  participantScreenShares.forEach((tr) => {
                    if (isRemotePublication(tr.publication))
                      void tr.publication.setSubscribed(true);
