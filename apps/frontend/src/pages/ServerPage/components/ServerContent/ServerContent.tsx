...
export const ServerContent = () => {
  ...
  const toggleChatSidebar = useStreamViewStore(
    (state: any) => state.toggleChatSidebar
  );
  const showStats = useStreamViewStore((state: any) => state.showStats);
  const toggleShowStats = useStreamViewStore(
    (state: any) => state.toggleShowStats
  );
  const fullscreen = useStreamViewStore((state: any) => state.fullscreen);
  const toggleFullscreen = useStreamViewStore(
    (state: any) => state.toggleFullscreen
  );
  const selectedSids = useStreamViewStore((state: any) => state.selectedSids);
  const screenShareTracksAll = allVideoTracks.filter(
    (t) => t.source === Track.Source.ScreenShare
  );
  const screenShareTracks =
    selectedSids && selectedSids.length > 0
      ? screenShareTracksAll.filter((t) =>
          selectedSids.includes(t.publication.trackSid)
        )
      : screenShareTracksAll;
  ...
  useEffect(() => {
    ...
    if (
      !primarySid ||
      !screenShareTracks.some((t) => t.publication.trackSid === primarySid)
    ) {
  ...
  useEffect(() => {
    ...
    const pub = screenShareTracksAll.find(
      (t) => t.publication.trackSid === primarySid
    )?.publication as any;
  ...
  const localTracks = screenShareTracksAll.filter(
    (t) => t.participant?.isLocal
  );

  const LocalPreviewUploader: React.FC<{ track: any }> = ({ track }) => {
    useUploadPreview(track);
    return null;
  };
  ...
  const getUsername = (participant: any) => {
    ...
    const member = server?.members?.find?.(
      (m: any) => m.userId === participant.identity
    );
  ...
  return (
    <Box sx={{ display: 'flex', height: '100%', minHeight: 0, flexGrow: 1 }}>
      ...
      {localTracks.map((t) => (
        <LocalPreviewUploader key={t.publication.trackSid} track={t} />
      ))}

      {/* Main content area */}
      <Box
        sx={{ position: 'relative', flexGrow: 1, minWidth: 0, minHeight: 0 }}
      >
                            ...
                            .map((track) => (
                              ...
                              >
                                <Box
                                  ...
                                  onClick={() =>
                                    setPrimarySid(track.publication.trackSid)
                                  }
                                >
                                  <ScreenSharePreview
                                    trackRef={track}
